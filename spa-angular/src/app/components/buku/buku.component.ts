...
export class BukuComponent implements OnInit {  // Deklarasi komponen dengan mengimplementasikan lifecycle hook OnInit
  ...
  getBuku(): void {  // Method untuk mengambil data fakultas dari API
  ...
  deleteBuku(_id: string): void {
    if (confirm('Apakah Anda yakin ingin menghapus data ini?')) { // Konfirmasi penghapusan
      ...
      this.http.delete(`${this.apiUrl}/${_id}`).subscribe({ //{ headers }
        next: () => {
  ...
  getBukuById(_id: string): void {
    ...
    this.http.get(`${this.apiUrl}/${_id}`).subscribe({
      next: (data: any) => {
        ...
        // Buka modal edit
        const modalElement = document.getElementById('editBukuModal') as HTMLElement;
        if (modalElement) {
  ...
  updateBuku(): void {
    if (this.bukuForm.valid) {
      ...
      this.http.put(`${this.apiUrl}/${this.editBukuId}`, this.bukuForm.value).subscribe({ //{ headers }
        next: (response) => {
  ...
  addBuku(): void {
    if (this.bukuForm.valid) {
      ...
      this.http.post(this.apiUrl, this.bukuForm.value).subscribe({ // { headers }
        next: (response) => {
